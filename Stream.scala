...
object Stream2 {
  def main(args: Array[String]) {
    ...
    val dfstreaming =spark.readStream.format("kafka")
      ...
      .option("kafka.ssl.truststore.location", "path/kafka.truststore.jks")
      .option("kafka.ssl.truststore.password" ,"mypassword" )
      .option("kafka.ssl.keystore.password" ,"mypassword")
      .option("kafka.ssl.key.password" ,"keypassword")
      .option("kafka.ssl.endpoint.identification.algorithm","")
      .option("kafka.ssl.keystore.location", "path\kafka01.keystore.jks")
      .option("kafka.sasl.jaas.config","org.apache.kafka.common.security.plain.PlainLoginModule required username='myusername' password='mypassword';")
      .load()
